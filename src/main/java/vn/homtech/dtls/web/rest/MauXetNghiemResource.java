...
import vn.homtech.dtls.repository.MauXetNghiemRepository;
import vn.homtech.dtls.repository.search.MauXetNghiemSearchRepository;
import vn.homtech.dtls.web.rest.errors.BadRequestAlertException;
import vn.homtech.dtls.web.rest.util.HeaderUtil;
import vn.homtech.dtls.web.rest.util.PaginationUtil;
...
public class MauXetNghiemResource {
    ...
    @Timed
    public ResponseEntity<List<MauXetNghiem>> getAllMauXetNghiems(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of MauXetNghiems");
        Page<MauXetNghiem> page;
        if (eagerload) {
            page = mauXetNghiemRepository.findAllWithEagerRelationships(pageable);
        } else {
            page = mauXetNghiemRepository.findAll(pageable);
        }
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, String.format("/api/mau-xet-nghiems?eagerload=%b", eagerload));
    ...
    public ResponseEntity<MauXetNghiem> getMauXetNghiem(@PathVariable Long id) {
        ...
        Optional<MauXetNghiem> mauXetNghiem = mauXetNghiemRepository.findOneWithEagerRelationships(id);
