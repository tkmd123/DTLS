...
export class DiemDotBienComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.diemDotBienService
                ...
                .subscribe(
                    (res: HttpResponse<IDiemDotBien[]>) => this.paginateDiemDotBiens(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/diem-dot-bien'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    private paginateDiemDotBiens(data: IDiemDotBien[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.queryCount = this.totalItems;
        this.diemDotBiens = data;
    }
