...
export class QuanHuyenComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.quanHuyenService
                ...
                .subscribe(
                    (res: HttpResponse<IQuanHuyen[]>) => this.paginateQuanHuyens(res.body, res.headers),
    ...
    private paginateQuanHuyens(data: IQuanHuyen[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.queryCount = this.totalItems;
        this.quanHuyens = data;
    }
